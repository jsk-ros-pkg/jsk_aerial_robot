  ...
  , navigator_loader_("aerial_robot_control", "aerial_robot_navigation::BaseNavigator")
{
  ...
  if (main_rate_ <= 0)
    ROS_ERROR_STREAM("main rate is negative, can not run the main timer");
  else
  {
...
void AerialRobotBase::mainFunc(const ros::TimerEvent& e)
{
  if (!e.last_real.isZero())
  {
    ...
    {
      // Log every 5 seconds
      ROS_WARN_THROTTLE(5.0, "main loop rate is too low: (ts_real) %f s > (ts_desire with %2f%% tolerance) %f s ",
