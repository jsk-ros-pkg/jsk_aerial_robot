...
class ImuData
{
 ...
 ImuData(ros::NodeHandle nh,
  ...
  void asctecGetImuDataArray(uint8_t * buffer, int buffer_size, 
                             ros::Time imuTimeStamp, float yaw_angle)
  {
    ...
    pitch = - rawPitch / 1000.0 * M_PI / 180;
    ...
    int rawYaw =
      ...
      + (int)(buffer[10] << 8)
      + (int)(buffer[11] & 0xff);
    yaw = - rawYaw / 1000.0 * M_PI / 180;

    //* acc x b (body frame) ; opposite!!
    short rawAccelerationX =
      (short)(buffer[12] << 8)
      + (short)(buffer[13] & 0xff);
    accelerationX = - rawAccelerationX /10000.0 * G;

    //* acc y b (body frame)
    short rawAccelerationY =
      (short)(buffer[14] << 8)
      + (short)(buffer[15] & 0xff);
    accelerationY = rawAccelerationY /10000.0 * G;

    //* acc z b (body frame)
    short rawAccelerationZ =
      (short)(buffer[16] << 8)
      + (short)(buffer[17] & 0xff);
    accelerationZ = rawAccelerationZ /10000.0 * G;

    //* calculate accTran
    //* gXb = -Sin(pitch) * G
    //float gXb = - sin(pitch) * (-G);
    //+*+*+**+ 慣性系
    accXc = (-accelerationX) * cos(pitch) + 
      (-accelerationY) * sin(pitch) * sin(roll) + 
      (-accelerationZ) * sin(pitch) * cos(roll);

    //* gYb = Cos(pitch) * Sin(roll) * G
    //float gYb = cos(pitch) * sin(roll) * (-G);
    accYc = (-accelerationY) * cos(roll) - (-accelerationZ) * sin(roll);

    //* gZb =  Cos(pitch) * Cos(roll) * G
    //float gZb = cos(pitch) * cos(roll) * (-G);
    ...
    int rawHeight =
      ...
      + (int)(buffer[20] << 8)
      + (int)(buffer[21] & 0xff);
    height = rawHeight / 1000.0;

    //* battey voltage
    short vBatTmp = (short)(buffer[22] << 8)
    ...
    accXw = cos(yaw_angle) * accXc - sin(yaw_angle) * accYc;
    accYw = sin(yaw_angle) * accXc + cos(yaw_angle) * accYc;

    accXwNonBias = cos(yaw_angle) * (accXc - accXBias) 
      - sin(yaw_angle) * (accYc -accYBias);
    accYwNonBias = sin(yaw_angle) * (accXc - accXBias) 
      + cos(yaw_angle) * (accYc -accYBias);
    ...
    if(kalmanFilterFlag)
      {
        ...
        imuXAxisNonBiasQuMsgPtr_->stamp = imuTimeStamp;
        ...
        jsk_quadcopter::ImuQuPtr imuYAxisNonBiasQuMsgPtr_
          = boost::shared_ptr<jsk_quadcopter::ImuQu> ( new jsk_quadcopter::ImuQu() );
        imuYAxisNonBiasQuMsgPtr_->stamp = imuTimeStamp;
        ...
        jsk_quadcopter::ImuQuPtr imuZAxisNonBiasQuMsgPtr_
          = boost::shared_ptr<jsk_quadcopter::ImuQu> ( new jsk_quadcopter::ImuQu() );
        imuZAxisNonBiasQuMsgPtr_->stamp = imuTimeStamp;
    ...
    if(kalmanFilterDebug)
      {
        if(kalmanFilterAxis == 0)
          {
            kf1_->prediction((double)accXw, imuTimeStamp);
            kf2_->prediction((double)accXw, imuTimeStamp);
          }
        else if(kalmanFilterAxis == 1)
          {

            kf1_->prediction((double)accYw, imuTimeStamp);
            kf2_->prediction((double)accYw, imuTimeStamp);

          }
      }
    publishImuData(imuTimeStamp, imuPub_);
  }
