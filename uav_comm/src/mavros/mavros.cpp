	...
	message_route_table {}
{
	...
	UAS_DIAG(&mav_uas).setHardwareID(std::string("ros"));

	ROS_INFO_STREAM("FCU via ros: ");
        fcu_link = boost::make_shared<MAVConnRos>(system_id, component_id);
	...
	mavlink_sub = mavlink_nh.subscribe("to", 100, &MavRos2::mavlink_sub_cb, this,
		ros::TransportHints()
	...
	mav_uas.sig_connection_changed.connect(boost::bind(&MavRos2::log_connect_change, this, _1));

	// connect FCU link
	fcu_link->message_received.connect(boost::bind(&MavRos2::mavlink_pub_cb, this, _1, _2, _3));
	fcu_link->message_received.connect(boost::bind(&MavRos2::plugin_route_cb, this, _1, _2, _3));
	fcu_link->port_closed.connect(boost::bind(&MavRos2::terminate_cb, this));
...
void MavRos2::mavlink_pub_cb(const mavlink_message_t *mmsg, uint8_t sysid, uint8_t compid) {
...
void MavRos2::mavlink_sub_cb(const mavros_msgs::Mavlink::ConstPtr &rmsg) {
...
void MavRos2::plugin_route_cb(const mavlink_message_t *mmsg, uint8_t sysid, uint8_t compid) {
 ...
 * @note Issue #257.
 */
bool MavRos2::is_blacklisted(std::string &pl_name, ros::V_string &blacklist, ros::V_string &whitelist) {
 ...
 */
void MavRos2::add_plugin(std::string &pl_name, ros::V_string &blacklist, ros::V_string &whitelist) {
...
void MavRos2::terminate_cb() {
	ROS_ERROR("FCU connection closed, mavros will be terminated.");
	ros::requestShutdown();
}

void MavRos2::startup_px4_usb_quirk(void) {
...
void MavRos2::log_connect_change(bool connected) {
