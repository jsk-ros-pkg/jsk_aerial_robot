...
//bakui
#define CREATE_SOURCE_ROUTE_API_LENGTH 11
...
namespace xbee{
  ...
  class CreateSourceRoute : public PayloadRequest {
  public:
    /**
     * Creates a unicast ZBTxRequest with the ACK option and DEFAULT_FRAME_ID
     */
    CreateSourceRoute(XBeeAddress64 &addr64, uint16_t addr16,uint8_t *payload, uint8_t payloadLength);
    /**
     * Creates a default instance of this class.  At a minimum you must specify
     * a payload, payload length and a destination address before sending this request.
     */
  ...
  public:
    ...
    /**
     * Creates a RemoteAtCommandRequest with 64-bit address and 16-bit address to set a command.
     * and applyChanges is true.
     */
    RemoteAtCommandRequest(XBeeAddress64 &remoteAddress64, uint16_t remoteAddress16, uint8_t *command, uint8_t *commandValue, uint8_t commandValueLength);
  ...
  class XBee {
  public:
    XBee(std::string port, uint32_t baudrate);
    ~XBee();
     ...
     */
    bool readPacket(double timeout);
    /**
  ...
  private:
    ...
    // last byte read
    //uint8_t b;
    uint8_t checksum_total_;
    uint8_t next_frame_id_;
    ...
    uint8_t response_frame_data_[MAX_FRAME_DATA_SIZE];

    void output (uint8_t &output, int len);
