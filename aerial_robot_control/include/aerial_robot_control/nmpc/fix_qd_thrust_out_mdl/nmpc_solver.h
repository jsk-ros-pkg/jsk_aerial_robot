...
public:
  FixQdMdlMPCSolver()
  {
    ...
    int status = fix_qd_thrust_out_mdl_acados_create(acados_ocp_capsule_);
    if (status)
      throw std::runtime_error("fix_qd_thrust_out_mdl_acados_create() returned status " + std::to_string(status) +
                               ". Exiting.");
