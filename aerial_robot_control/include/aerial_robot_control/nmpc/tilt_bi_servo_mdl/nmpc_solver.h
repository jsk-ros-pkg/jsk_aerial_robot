...
public:
  TiltBiServoMdlMPCSolver()
  {
    ...
    int status = tilt_bi_servo_mdl_acados_create_with_discretization(acados_ocp_capsule_, NN_, new_time_steps);
    if (status)
      throw std::runtime_error("tilt_bi_servo_mdl_acados_create_with_discretization() returned status " +
...
protected:
  ...
  {
    return tilt_bi_servo_mdl_acados_update_params_sparse(acados_ocp_capsule_, stage, idx, p, n_update);
  }
