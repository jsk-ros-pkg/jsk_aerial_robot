...
#ifndef TILT_MT_NEURAL_SERVO_MINUS_MPC_CONTROLLER_H
#define TILT_MT_NEURAL_SERVO_MINUS_MPC_CONTROLLER_H
...
class TiltMtNeuralServoMinusMPC : public BaseMPC
{
public:
  TiltMtNeuralServoMinusMPC() = default;  // note that constructor shouldn't have arguments as the rule of rospluginlib
  ~TiltMtNeuralServoMinusMPC() override = default;
...
protected:
  ...
  virtual void initMPCParams();
  void updateInertialParams();
  std::vector<double> PhysToMPCParams() const;
...
#endif  // TILT_MT_NEURAL_SERVO_MINUS_MPC_CONTROLLER_H
