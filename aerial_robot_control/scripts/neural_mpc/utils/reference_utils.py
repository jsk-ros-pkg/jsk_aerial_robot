...
def sample_random_position_target(current_pos, world_radius, aggressive=True, low_flight=False):
    ...
    :param low_flight: Flag to shift targets near to the ground - to be used for data recording with ground effect.
    ...
    if low_flight:
        if np.random.random() < 0.8:
            ...
            target_pos[2] = target_pos[2] / world_radius
...
def sample_random_orientation_target(aggressive=True, low_flight=False):
    """
    Generates a target pose for the robot to reach.
    ...
    if aggressive:
        target_rot = np.random.uniform(-np.pi / 1.5, np.pi / 1.5, (3,))
    else:
        target_rot = np.random.uniform(-np.pi / 4, np.pi / 4, (3,))
    target_omega = np.array([0, 0, 0])
