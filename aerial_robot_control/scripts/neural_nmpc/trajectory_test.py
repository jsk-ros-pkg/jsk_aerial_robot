...
def main(args):
    ...
    simulation_options = EnvConfig.sim_options["disturbances"]

    debug_plots = EnvConfig.pre_run_debug_plots
    tracking_results_plot = EnvConfig.result_plots
    sim_gui = EnvConfig.custom_sim_gui
    ...
    if sim_gui:
        ...
        real_time_artists = initialize_drone_plotter(
            n_props=n_mpc_nodes, quad_rad=my_quad.length, world_rad=world_radius, full_traj=reference_traj
        )
    ...
    if tracking_results_plot:
        ...
        trajectory_tracking_results(
            reference_timestamps, reference_traj, quad_trajectory, reference_u, u_optimized_seq, title
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_type", type=str, default="gp", help="Type of regression model (GP or RDRv linear)")
    ...
    input_arguments = parser.parse_args(
        ["--model_version", "5f15661", "--model_name", "simple_sim_mlp", "--model_type", "mlp_approx"]
    )
