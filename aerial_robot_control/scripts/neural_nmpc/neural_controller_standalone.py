...
class NeuralNMPC(RecedingHorizonBase):
    def __init__(
        ...
        if model_options["nmpc_type"] != "NMPCTiltQdServo":
            raise NotImplementedError(
                "Model flags, Model name, Parameter loading, Weights and Reference generation "
        ...
        self.T_samp = self.params["T_samp"]  # Sampling time for the NMPC controller, time step between two steps
        ...
        if self.arch_type == "tilt_qd":
            self.tilt = True
            if self.nmpc_type == "NMPCTiltQdServo":
        ...
        super().__init__(method="nmpc", build=True)
    ...
    def create_acados_model(self) -> AcadosModel:
        ...
        if self.use_mlp:
            ...
            if self.model_options["end_to_end_mlp"]:
                ...
                M = get_output_mapping(state.shape[0], self.y_reg_dims)
                O = get_inverse_output_mapping(state.shape[0], self.y_reg_dims)

                # Combine nominal dynamics with neural dynamics
                ds_total = O @ ds + M @ mlp_out
            else:
                ...
                # Combine nominal dynamics with neural dynamics
                ds_total = ds + M @ mlp_out
                # ds_total = nominal_dynamics - M @ mlp_out
        ...
        f = ca.Function("f", [state, controls], [ds_total], ["state", "control_input"], ["ds"], {"allow_free": True})
    ...
    def create_acados_ocp_solver(self, build: bool = True) -> AcadosOcpSolver:
        ...
        if self.include_quaternion_constraint:
            ...
            # and rather reduce solving performance due to higher dimensionality of the problem.
