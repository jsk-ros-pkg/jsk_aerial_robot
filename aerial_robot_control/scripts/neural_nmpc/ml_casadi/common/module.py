    ...
    def approx(self, x: cs.MX, order=1, parallel=False):
        ...
        if order == 1:
            ...
            if parallel:
                ...
                second_order_term_oi_fun = cs.Function(
                    "second_order_term_fun",
                ...
                second_order_term = 0.5 * cs.vcat(
                    [cs.mtimes(cs.transpose(x_minus_a), cs.mtimes(f_ddf_a, x_minus_a)) for f_ddf_a in f_ddf_as]
                )

            return f_f_a + cs.mtimes(f_df_a, x_minus_a) + second_order_term
        else:
            raise ValueError("Only implemented for first and second order.")
