...
def main(model_options, solver_options, dataset_options, sim_options, run_options):
    """
    IDEA: Control with nominal model, simulate with neural model (trained on real data), record data for training
    """
    ...
    sim_solver_neural = rtnmpc_neural.create_acados_sim_solver(T_sim)

    # Undisturbed model for creating labels to train on
    dynamics_forward_prop, state_forward_prop, u_forward_prop = init_forward_prop(rtnmpc_nominal.nmpc)
    ...
    if recording:
        ...
        model_options["include_quaternion_constraint"] = rtnmpc_nominal.nmpc.include_quaternion_constraint
        model_options["include_soft_constraints"] = rtnmpc_nominal.nmpc.include_soft_constraints
        model_options["nmpc_params"] = rtnmpc_nominal.nmpc.params
        if rtnmpc_nominal.use_mlp:
            model_options["delay_horizon"] = rtnmpc_nominal.mlp_metadata["MLPConfig"]["delay_horizon"]
    ...
    while False in targets_reached:
        ...
        while not current_target_reached:
            ...
            if u_cmd_nominal is None:
                ...
                rtnmpc_neural.check_state_constraints(state_curr_neural, i)
            ...
            comp_time = time.time()
            u_cmd_nominal = ocp_solver_nominal.solve_for_x0(state_curr_neural)
            ...
            if plot:  # or recording:
                ...
                rec_dict["control"] = np.append(rec_dict["control"], u_cmd_nominal[np.newaxis, :], axis=0)
            ...
            while simulation_time < T_samp:
                ...
                state_curr_sim_neural = sim_solver_neural.simulate(
                    x=state_curr_sim_neural, u=u_cmd_nominal, p=rtnmpc_neural.sim_acados_parameters
                )
    ...
    if plot and not recording:
        plot_trajectory(model_options, rec_dict, rtnmpc_nominal)
        halt = 1
        plt.show()
