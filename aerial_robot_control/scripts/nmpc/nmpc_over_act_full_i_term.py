...
def create_acados_model() -> AcadosModel:
    model_name = "beetle_full_w_disturb_model"
    ...
    for i in range(N_sim):
        ...
        if t_ctl >= nmpc_params["T_samp"]:
            ...
            for j in range(acados_ocp_solver.N):
                ...
                params = np.concatenate((quaternion_r, np.zeros(6)))
                acados_ocp_solver.set(j, "p", params)  # for nonlinear quaternion error
            ...
            params = np.concatenate((quaternion_r, np.zeros(6)))
            acados_ocp_solver.set(acados_ocp_solver.N, "p", params)  # for nonlinear quaternion error
