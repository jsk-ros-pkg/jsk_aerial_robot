...
#include "aerial_robot_control/nmpc/fix_qd_angvel_out_mdl/nmpc_solver.h"
...
void nmpc_under_act_body_rate::MPCSolver::initialize(PhysicalParams& phys_params)
{
  ...
  acados_ocp_capsule_ = fix_qd_angvel_out_mdl_acados_create_capsule();

  new_time_steps = nullptr;

  int status = fix_qd_angvel_out_mdl_acados_create_with_discretization(acados_ocp_capsule_, NN, new_time_steps);
  if (status)
  {
    ROS_WARN("fix_qd_angvel_out_mdl_acados_create() returned status %d. Exiting.\n", status);
    exit(1);
  }

  nlp_config_ = fix_qd_angvel_out_mdl_acados_get_nlp_config(acados_ocp_capsule_);
  nlp_dims_ = fix_qd_angvel_out_mdl_acados_get_nlp_dims(acados_ocp_capsule_);
  nlp_in_ = fix_qd_angvel_out_mdl_acados_get_nlp_in(acados_ocp_capsule_);
  nlp_out_ = fix_qd_angvel_out_mdl_acados_get_nlp_out(acados_ocp_capsule_);
  nlp_solver_ = fix_qd_angvel_out_mdl_acados_get_nlp_solver(acados_ocp_capsule_);
  nlp_opts_ = fix_qd_angvel_out_mdl_acados_get_nlp_opts(acados_ocp_capsule_);
  ...
  for (int i = 0; i < NN; i++)
  {
    fix_qd_angvel_out_mdl_acados_update_params(acados_ocp_capsule_, i, p, NP);
  }
  fix_qd_angvel_out_mdl_acados_update_params(acados_ocp_capsule_, NN, p, NP);
...
{
  // 1. free solver
  int status = fix_qd_angvel_out_mdl_acados_free(acados_ocp_capsule_);
  if (status)
    ROS_WARN("fix_qd_angvel_out_mdl_acados_free() returned status %d. \n", status);

  // 2. free solver capsule
  status = fix_qd_angvel_out_mdl_acados_free_capsule(acados_ocp_capsule_);
  if (status)
    ROS_WARN("fix_qd_angvel_out_mdl_acados_free_capsule() returned status %d. \n", status);
}
                                                       ...
                                                       const unsigned int x_stride, const unsigned int u_stride)
{
  ...
  for (int i = 0; i < NN; i++)
  {
    ...
    fix_qd_angvel_out_mdl_acados_update_params_sparse(acados_ocp_capsule_, i, qr_idx, qr, 4);
  }
  // final x and p, no u
  double xr[NX];
  ...
  fix_qd_angvel_out_mdl_acados_update_params_sparse(acados_ocp_capsule_, NN, qr_idx, qr, 4);
}
...
double nmpc_under_act_body_rate::MPCSolver::solveOCPInLoop(const int N_timings)
{
  ...
  {
    int status = fix_qd_angvel_out_mdl_acados_solve(acados_ocp_capsule_);
    if (status != ACADOS_SUCCESS)
    {
      std::cout << "fix_qd_angvel_out_mdl_acados_solve() returned status " << status << ".\n";
    }
...
void nmpc_under_act_body_rate::MPCSolver::printStatus(const int N_timings, const double min_time)
{
  ...
  fix_qd_angvel_out_mdl_acados_print_stats(acados_ocp_capsule_);
  ROS_DEBUG("\nSolver info:\n");
