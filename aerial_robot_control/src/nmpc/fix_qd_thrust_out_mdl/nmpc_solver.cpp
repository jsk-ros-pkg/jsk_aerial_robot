...
void nmpc_under_act_full::MPCSolver::initialize(Constraints& constraints)
{
  ...
  int status = fix_qd_thrust_out_mdl_acados_create_with_discretization(acados_ocp_capsule_, NN, new_time_steps);
  if (status)
  {
    ROS_WARN("fix_qd_thrust_out_mdl_acados_create() returned status %d. Exiting.\n", status);
    exit(1);
  }
  ...
  for (int i = 0; i < NN; i++)
  {
    fix_qd_thrust_out_mdl_acados_update_params(acados_ocp_capsule_, i, p, NP);
  }
  fix_qd_thrust_out_mdl_acados_update_params(acados_ocp_capsule_, NN, p, NP);
...
nmpc_under_act_full::MPCSolver::~MPCSolver()
{
  ...
  if (status)
    ROS_WARN("fix_qd_thrust_out_mdl_acados_free() returned status %d. \n", status);

  // 2. free solver capsule
  status = fix_qd_thrust_out_mdl_acados_free_capsule(acados_ocp_capsule_);
  if (status)
    ROS_WARN("fix_qd_thrust_out_mdl_acados_free_capsule() returned status %d. \n", status);
}
                                                  ...
                                                  const unsigned int u_stride)
{
  ...
  for (int i = 0; i < NN; i++)
  {
    ...
    fix_qd_thrust_out_mdl_acados_update_params_sparse(acados_ocp_capsule_, i, qr_idx, qr, 4);
  }
  // final x and p, no u
  double xr[NX];
  ...
  fix_qd_thrust_out_mdl_acados_update_params_sparse(acados_ocp_capsule_, NN, qr_idx, qr, 4);
}
...
double nmpc_under_act_full::MPCSolver::solveOCPInLoop(const int N_timings)
{
  ...
  {
    int status = fix_qd_thrust_out_mdl_acados_solve(acados_ocp_capsule_);
    if (status != ACADOS_SUCCESS)
    {
      std::cout << "fix_qd_thrust_out_mdl_acados_solve() returned status " << status << ".\n";
    }
