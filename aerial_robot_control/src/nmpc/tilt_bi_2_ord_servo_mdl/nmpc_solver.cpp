...
nmpc_tilt_bi_2_ord::MPCSolver::MPCSolver()
{
}

void nmpc_tilt_bi_2_ord::MPCSolver::initialize()
{
  ...
  int status = tilt_bi_2_ord_servo_mdl_acados_create_with_discretization(acados_ocp_capsule_, NN, new_time_steps);
  if (status)
  {
    ROS_WARN("tilt_bi_2_ord_servo_mdl_acados_create() returned status %d. Exiting.\n", status);
    exit(1);
  }
  ...
  for (int i = 0; i < NN; i++)
  {
    tilt_bi_2_ord_servo_mdl_acados_update_params(acados_ocp_capsule_, i, p, NP);
  }
  tilt_bi_2_ord_servo_mdl_acados_update_params(acados_ocp_capsule_, NN, p, NP);
...
nmpc_tilt_bi_2_ord::MPCSolver::~MPCSolver()
{
  ...
  if (status)
    ROS_WARN("tilt_bi_2_ord_servo_mdl_acados_free() returned status %d. \n", status);

  // 2. free solver capsule
  status = tilt_bi_2_ord_servo_mdl_acados_free_capsule(acados_ocp_capsule_);
  if (status)
    ROS_WARN("tilt_bi_2_ord_servo_mdl_acados_free_capsule() returned status %d. \n", status);
}

void nmpc_tilt_bi_2_ord::MPCSolver::reset(const aerial_robot_msgs::PredXU& x_u)
{
...
void nmpc_tilt_bi_2_ord::initPredXU(aerial_robot_msgs::PredXU& x_u)
{
                                                 ...
                                                 const unsigned int u_stride)
{
  ...
  for (int i = 0; i < NN; i++)
  {
    ...
    tilt_bi_2_ord_servo_mdl_acados_update_params_sparse(acados_ocp_capsule_, i, qr_idx, qr, 4);
  }
  // final x and p, no u
  double xr[NX];
  ...
  tilt_bi_2_ord_servo_mdl_acados_update_params_sparse(acados_ocp_capsule_, NN, qr_idx, qr, 4);
}
...
double nmpc_tilt_bi_2_ord::MPCSolver::solveOCPOnce()
{
  double min_time = 1e12;
  double elapsed_time;

  int status = tilt_bi_2_ord_servo_mdl_acados_solve(acados_ocp_capsule_);
  if (status != ACADOS_SUCCESS)
  {
    ROS_WARN("tilt_bi_2_ord_servo_mdl_acados_solve() returned status %d.\n", status);
  }
...
void nmpc_tilt_bi_2_ord::MPCSolver::getSolution(const unsigned int x_stride, const unsigned int u_stride)
{
  for (int i = 0; i < NN; i++)
  {
...
void nmpc_tilt_bi_2_ord::MPCSolver::printSolution()
{
  std::stringstream ss;
...
void nmpc_tilt_bi_2_ord::MPCSolver::printStatus(const double min_time)
{
  double kkt_norm_inf;
  int sqp_iter;
...
void nmpc_tilt_bi_2_ord::MPCSolver::setCostWDiagElement(int index, double value, bool is_set_WN) const
{
  if (index < nx_ + nu_)
...
void nmpc_tilt_bi_2_ord::MPCSolver::setCostWeight(bool is_update_W, bool is_update_WN)
{
  if (is_update_W)
  {
    for (int i = 0; i < NN; i++)
