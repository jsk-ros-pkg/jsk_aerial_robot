...
nmpc_over_act_no_servo_delay::MPCSolver::~MPCSolver()
{
  ...
  if (status)
    std::cout << "beetle_no_servo_delay_model_acados_free() returned status " << status << ".\n";

  // 2. free solver capsule
  status = beetle_no_servo_delay_model_acados_free_capsule(acados_ocp_capsule_);
  if (status)
    std::cout << "beetle_no_servo_delay_model_acados_free_capsule() returned status " << status << ".\n";
}
