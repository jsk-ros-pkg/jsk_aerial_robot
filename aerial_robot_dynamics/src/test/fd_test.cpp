  ...
  for(int i = 0; i < robot_model_->getRotorNum(); i++)
    {
      ...
      pinocchio::computeFrameJacobian(*(robot_model_->getModel()), *(robot_model_->getData()), q, robot_model_->getModel()->getFrameId("rotor" + std::to_string(i+1)), pinocchio::LOCAL, rotor_i_jacobian); // LOCAL
      ...
      rotor_i_wrench.tail<3>() = Eigen::Vector3d(0, 0, robot_model_->getMFRate() * thrust(i));
