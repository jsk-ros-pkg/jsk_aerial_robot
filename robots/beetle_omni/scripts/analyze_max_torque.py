'''
 Created by li-jinjie on 25-5-13.
'''
import numpy as np
import cvxpy as cp
import argparse
...
def find_torque_thrust_for_orientation(alloc_mtx, fg_w,
                                       roll_deg, pitch_deg, yaw_deg,
                                       search_torque_min, search_torque_max,
                                       is_world,
    ...
    lo, hi = search_torque_min, search_torque_max
    best_torque = search_torque_min
    best_error = np.inf
    ...
    for iter_idx in range(max_iters):
        ...
        # target wrench: include gravity plus additional torque along body z
        if is_world:
            tgt_w = np.array([[fg_b[0], fg_b[1], fg_b[2], 0.0, 0.0, mid]]).T  # the z-axis is torque
        else:
            mid_b = R_bw @ np.array([0.0, 0.0, mid])
            tgt_w = np.array([[fg_b[0], fg_b[0], fg_b[0], mid_b[0], mid_b[1], mid_b[2]]]).T
        ok, err = check_wrench_available(alloc_mtx, tgt_w)
        if ok:
            best_torque, best_error = mid, err
            lo = mid  # can try larger torque
        else:
            hi = mid  # reduce torque
        if hi - lo < tol:
            break

    if iter_idx == max_iters - 1:
        print(f"Warning: max iterations reached ({max_iters}) for torque search.")
        print(f"Final torque range: [{lo}, {hi}]")

    return best_torque, best_error


if __name__ == "__main__":
    # add arguments to choose 1. resolution 2. the type is world or body
    parser = argparse.ArgumentParser(description="Analyze maximum torque for a given pitch/yaw orientation.")
    ...
    # 3) compute max torque map
    torque_map = np.zeros((len(yaw_list), len(pitch_list)))
    for i, yaw_deg in enumerate(yaw_list):
        for j, pitch_deg in enumerate(pitch_list):
            search_torque_limit = 6 * THRUST_MAX * np.sqrt(p1_b[0] ** 2 + p1_b[1] ** 2)
            torque_map[i, j], _ = find_torque_thrust_for_orientation(
                alloc_mat, fg_w, 0.0, pitch_deg, yaw_deg, 0.0, search_torque_limit, if_world)
        print(f"Completed yaw = {yaw_deg:.1f}°")

    if True:
        # save torque map to file
        np.savez("torque_map.npz", yaw_list=yaw_list, pitch_list=pitch_list, torque_map=torque_map)
        print("Saved torque map to torque_map.npz")
    ...
    norm = plt.Normalize(torque_map.min(), torque_map.max())
    colors = cm.plasma(norm(torque_map))
    ...
    ax.set_xlabel(f'$^{coord_text}\\tau_x$ [N$\\cdot$m]', fontsize=label_size)
    ax.set_ylabel(f'$^{coord_text}\\tau_y$ [N$\\cdot$m]', fontsize=label_size)
    ax.set_zlabel(f'$^{coord_text}\\tau_z$ [N$\\cdot$m]', fontsize=label_size)
    ...
    # save the figure
    file_name = "max_torque_world_3d.pdf" if if_world else "max_torque_body_3d.pdf"
    ...
    ax2.set_xlabel(f'$^{coord_text}\\tau_x$ [N$\\cdot$m]', fontsize=label_size)
    ax2.set_ylabel(f'$^{coord_text}\\tau_z$ [N$\\cdot$m]', fontsize=label_size)
    ...
    cb.set_label("Torque [N$\\cdot$m]", fontsize=label_size)
