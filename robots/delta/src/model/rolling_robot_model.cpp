...
void RollingRobotModel::updateRobotModelImpl(const KDL::JntArray& joint_positions)
{
  ...
  // const std::vector<Eigen::Vector3d> rotor_p = getRotorsOriginFromCog<Eigen::Vector3d>();
  // const std::vector<Eigen::Vector3d> rotor_u = getRotorsNormalFromCog<Eigen::Vector3d>();
  // const auto& sigma = getRotorDirection();
  // const double m_f_rate = getMFRate();
  // wrench_allocation_matrix_from_target_frame_.resize(6, getRotorNum());
  // for(int i = 0; i < getRotorNum(); i++)
  //   {
  //     Eigen::Vector3d p_i_from_target_frame = kdlToEigen(getTargetFrame().Inverse() * cog) * rotor_p.at(i);
  //     Eigen::Vector3d u_i_from_target_frame = kdlToEigen(getTargetFrame().M.Inverse() * cog.M) * rotor_u.at(i);
  //     wrench_allocation_matrix_from_target_frame_.block(0, i, 3, 1) = u_i_from_target_frame;
  //     wrench_allocation_matrix_from_target_frame_.block(3, i, 3, 1) = p_i_from_target_frame.cross(u_i_from_target_frame) + m_f_rate * sigma.at(i + 1) * u_i_from_target_frame;
  //   }
