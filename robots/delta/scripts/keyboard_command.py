...
if __name__=="__main__":
        ...
        standup_demo_pub = rospy.Publisher(robot_ns + '/standup_demo', Empty, queue_size=1)
        rolling_demo_pub = rospy.Publisher(robot_ns + '/rolling_demo', Empty, queue_size=1)
        ...
        try:
                while(True):
                        ...
                        if key == 'p':
                                ...
                                ctrl_mode_pub.publish(comm)
                        if key == 's':
                            standup_demo_pub.publish(Empty())
                        if key == 'd':
                            rolling_demo_pub.publish(Empty())
                        if key == '\x03':
                                break
                        rospy.sleep(0.001)
