...
protected:
  ...
  HydrusNumericalJacobian* numerical_sovler_;
  ...
  virtual void SetUp()
  {
    ...
    numerical_sovler_ = new HydrusNumericalJacobian(nh_, nhp_, std::move(std::make_unique<HydrusRobotModel>(true)));
...
int main(int argc, char **argv)
{
  ...
  if(rostest)
    ...
    {
      HydrusNumericalJacobian numerical_sovler(nh, nhp, std::move(std::make_unique<HydrusRobotModel>(true)));
      ros::spin();
    }
}
